...
class FunctorLawTester:  # pragma: no cover
    ...
    def __init__(self, functor, mapper1, mapper2, call_fn=identity):
    ...
    def identity_law_test(self):
        assert self.call_fn(self.functor.map(identity)) == self.call_fn(self.functor)

    def composition_law_test(self):
        mapped_functor1 = self.call_fn(self.functor.map(self.mapper1).map(self.mapper2))
        mapped_fuctor2 = self.call_fn(
            self.functor.map(lambda value: self.mapper2(self.mapper1(value)))
        )
