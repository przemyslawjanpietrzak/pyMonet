...
def test_try_should_appied_bind_when_exception_not_was_thrown():

    def fail_callback(_):
        assert True is False

    (Try.of(divide, 42, 2)
        .bind(lambda value: Try.of(divide, 42, 3))
        .on_success(lambda value: value == 7)
        .on_fail(fail_callback))


def test_try_should_not_appied_bind_when_exception_was_thrown():
